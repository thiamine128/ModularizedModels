...
class CalculateFLOP:
    ...
    def calculate_flop_fvcore(self, img_size, is_sparse, show_details=False):
        ...
        if is_sparse:
            handlers = {'aten::_convolution': self.masked_conv_flop_jit,
                        ...
                        'aten::matmul': self.masked_linear_mm_flop_jit}
    ...
    def masked_fc_flop_jit(self, inputs: List[Any], outputs: List[Any]) -> Number:
        ...
        return flop
    def masked_linear_mm_flop_jit(self, inputs: List[Any], outputs: List[Any]) -> Number:
        """
        Count flops for linear layers.
        """
        # Count flop for nn.Linear
        # inputs is a list of length 3.
        input_shapes = [self.get_shape(v) for v in inputs]
        # input_shapes[0]: [batch size, input feature dimension]
        # input_shapes[1]: [batch size, output feature dimension]

        # For vgg16 and resnet20 model. other models may use different inputs[x].
        fc_name = re.match(r'.+scope: /(.+) #.*', str(inputs[1]).strip()).groups(1)[0].strip()
        fc_name = fc_name.split('/')[-1]

        weight =  self.model_modules[fc_name].weight.numpy()
        #print(input_shapes[0], input_shapes[1], weight.shape)
        #assert input_shapes[1] == weight.shape
        flop = np.prod(input_shapes[0][:-1]) * np.count_nonzero(weight)
